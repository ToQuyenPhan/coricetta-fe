...
function RecipeDetail() {
  ...
  const fetchReportedRecipeData = async () => {
    const res = await fetch(
      `https://localhost:44327/api/Reports/find?userId=${userId}&recipeId=${recipeId}`,
      {
        mode: "cors",
        method: "GET",
        headers: new Headers({
  ...
  const fetchLikeData = async () => {
    const res = await fetch(
      `https://localhost:44327/api/Actions/getAction?userId=${userId}&recipeId=${recipeId}&Type=0`,
      {
        mode: "cors",
        method: "GET",
        headers: new Headers({
  ...
  const fetchShareData = async () => {
    const res = await fetch(
      `https://localhost:44327/api/Actions/getAction?userId=${userId}&recipeId=${recipeId}&Type=3`,
      {
        mode: "cors",
        method: "GET",
        headers: new Headers({
  ...
  let config = {
    method: "get",
    maxBodyLength: Infinity,
    url: `https://localhost:44327/api/Menus/allByUserId?userId=${userId}&currentPage=1&pageSize=10`,
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
      Accept: "application/json",
  ...
  axios
    .request(config)
    .then((response) => {
      console.log("dataMenu", JSON.stringify(response.data));
  ...
  const handleDeleteClick = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await fetch(
          `https://localhost:44327/api/Recipes/delete?recipeid=${recipeId}`,
          {
            mode: "cors",
            method: "DELETE",
            headers: new Headers({
  ...
  const handleDislikeClick = async (event) => {
    event.preventDefault();
    const res = await fetch(
      `https://localhost:44327/api/Actions?actionId=${like.id}`,
      {
        mode: "cors",
        method: "DELETE",
        headers: new Headers({
  ...
  const AddToMenu = (menuId,recipeId) => {
    ...
    const res = fetch(
      `https://localhost:44327/api/Menus/addRecipe?menuId=${menuId}&recipeId=${recipeId}`,
      {
        mode: "cors",
        method: "POST",
        headers: new Headers({
    ...
    if (res.status === 200) {
      Swal.fire({
        position: "center",
        icon: "success",
        title: "Thêm Vào menu thành công!",
        showConfirmButton: false,
        timer: 1500,
      });
      navigate("/home");
  ...
  const handleOpenAddToMenu = () => setOpenAddToMenu(!openAddToMenu);
                    ...
                    <div className="single-blog-item">
                      <div className="blog-item-content p-5">
                        ...
                        {parseInt(userId) !== parseInt(authorId) ? (
                          <div className="w-fit float-right mr-5">
                              ...
                              {isReported ? (
                                <div
                                  variant="gradient"
                                  className="shadow-none text-red-600"
                                >
                                      ...
                                      >
                                        <Popover.Panel className="absolute right-0 mt-2 w-48 overflow-hidden bg-white rounded-lg shadow-lg ring-1 ring-gray-300">
                                          <Button
                                            onClick={handleOpen}
                                            variant="gradient"
                                            className="shadow-none text-yellow-400 hover:text-red-600 flex justify-center items-center 
                                hover:cursor-pointer"
                                          ...
                                          <Button
                                            onClick={() =>
                                              handleShoppingClick(recipe.id)
                                            }
                                            variant="gradient"
                                            className="shadow-none text-black flex justify-center items-center 
                        ...
                        hover:cursor-pointer hover:text-gray-600"
                              ...
                              >
                                <DialogHeader>
                                  <h2 className="font-bold text-center w-full text-orange-600">
                                    Báo cáo công thức
                                  </h2>
                                </DialogHeader>
                                <DialogBody divider>
                                  <form
                                    id="report"
                                    onSubmit={fetchReportRecipeData}
                                  >
                                    <div className="mb-4">
                                      <h5 className="text-left ml-3 font-bold">
                                        Mô tả:
                                      </h5>
                                      <input
                                        className="border border-gray-300 p-3 w-full rounded font-sans text-base text-black focus:outline-0"
                                        type="text"
                                        placeholder="Nhập mô tả về hành vi của công thức!"
                        ...
                        hover:cursor-pointer hover:text-gray-600"
                                      ...
                                      <Button
                                        ...
                                        variant="gradient"
                                        className="shadow-none text-red-600 flex justify-center items-center 
                        hover:cursor-pointer hover:text-gray-600"
                                      >
                                        <span>Xóa</span>
                                ...
                                <DialogBody divider>
                                  ...
                                  >
                                    <div className="mb-4">
                                      ...
                                      {mymenu !== null ? (
                                      <Select className="" options={mymenu?.items} placeholder="Chọn menu!"
                                                        value={selectedMenu} onChange={handleSelectMenu} />
                                      ) : (<div>Bạn chưa có menu</div>)}
                                    </div>
                                  </form>
                                </DialogBody>
                                <DialogFooter className="flex justify-end">
                                  ...
                                  <Button
                                    ...
                                    className=" bg-red-600 px-3 py-1"
                                    form="report"
                                    onClick={()=>AddToMenu(selectedMenu.value, recipe.id)}
                                  >
                              ...
                              <div>
                                {isLiked ? (
                                  <Button
                                    ...
                                    variant="gradient"
                                    className="shadow-none text-blue-800 flex justify-center items-center 
